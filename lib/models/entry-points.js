...
class EntryPoints {
  ...
  constructor (client, options) {
    ...
    // this.db.createCollection(EntryPoints.collectionName)
   ...
   * @param {string} entryPoint.name - Name of the entry point
   * @param {Object} [options] - Options
   ...
   * @param {string} lotId - Lot Id
   * @param {Object} options - Options
   * @param {Function} [options.iterator] - Custom iterator
   * @param {*} [options.accumulator=[]] - Custom accumulator
   * @returns {Promise<Object[]>} - Entry points for the given lot
   */
  async list (lotId, options = {}) {
    const {
      accumulator = [],
      iterator
    } = options
    ...
    if (iterator !== undefined) {
      await cursor.forEach(iterator.bind(null, accumulator))
