...
class Invoices {
  ...
  constructor (client, options) {
    ...
    // this.db.createCollection(Invoices.collectionName)
  ...
  /**
   * Retrieves the invoice of given slot Id
   * @param {string} slotId - Slot Id
   * @param {Object} options - Options
   * @param {boolean} [options.settled=false] - Settled query
   * @returns {Promise<Object>} - Invoice
   */
  async getBySlot (slotId, options = {}) {
    const {
      settled = false
    } = options
  ...
  /**
   * Retrieves any unsettled invoice of given vin
   * @param {string} vin - VIN
   * @returns {Promise<Object>} - Unsettled invoice
   */
  async getUnsettledInvoice (vin) {
    return this.collection.findOne({
      vin,
      settled: false
    })
